...
public class DebuggerItem extends Item {
    ...
    public void debugAction(int selection, PlayerEntity user) {
        if (selection == 20) {
            user.sendMessage(Text.of(user.getDataTracker().get(MiscUtil.IS_UNDER_FALL_PROTECTION).toString()));
            user.getDataTracker().set(MiscUtil.IS_UNDER_FALL_PROTECTION, true);
            user.sendMessage(Text.of(user.getDataTracker().get(MiscUtil.IS_UNDER_FALL_PROTECTION).toString()));
            return;
