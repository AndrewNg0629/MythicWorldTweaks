...
public class TrialSpawnerStateMixin {
    ...
    private boolean wrapAreMobsDead(TrialSpawnerData instance, Operation<Boolean> original, @Local(argsOnly = true) TrialSpawnerLogic logic, @Local(argsOnly = true) ServerWorld world) {
        ...
        if (ConfigManager.getConfig().tweaks().valueTweaks().trialSpawnerCooldownOverride().enabled() && allDead) {
            logic.getData().mythicworldtweaks$setCompletionTimestamp(world.getTime());
        }
        return allDead;
    }
}
